#include "vanjee_mini_lidar/vanjee_718mini_node.h"

namespace vanjee_lidar
{
    Vanjee718MiniNode::Vanjee718MiniNode()
    {
        ros::NodeHandle nh("~");
        ...
        lidar_protocol_ = new Vanjee718MiniLidarProtocol();
        dynamic_reconfigure::Server<vanjee_mini_lidar::vanjee_718mini_lidarConfig>::CallbackType f;
        f = boost::bind(&Vanjee718MiniNode::callback, this, _1, _2);
    ...
    void Vanjee718MiniNode::callback(vanjee_mini_lidar::vanjee_718mini_lidarConfig &config, uint32_t level)
    {
        lidar_protocol_->setConfig(config, level);
    }

    void Vanjee718MiniNode::connect()
    {
        ...
        while (!commumication_->connected_)
        {
            ...
            {
                ROS_INFO("Succesfully connected. Hello vanjee_718mini_lidar!");
            }
            else
            {
    ...
    void Vanjee718MiniNode::checkConnection()
    {
...
{
    ros::init(argc, argv, "vanjee_718mini_lidar_01");
    
    vanjee_lidar::Vanjee718MiniNode *vanjee_node = new vanjee_lidar::Vanjee718MiniNode();

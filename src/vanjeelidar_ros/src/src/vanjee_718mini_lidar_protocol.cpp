#include "vanjee_mini_lidar/vanjee_718mini_lidar_protocol.h"

namespace vanjee_lidar
{
    Vanjee718MiniLidarProtocol::Vanjee718MiniLidarProtocol()
    {
        freq_scan_ = 1;
        pre_frame_no_ = 0;

        scan_.header.frame_id = "vanjee_718mini_lidar_frame";
        ...
        std::cout << "vanjee_718mini_protocl start success" << std::endl;
    }
    ...
    bool Vanjee718MiniLidarProtocol::setConfig(vanjee_mini_lidar::vanjee_718mini_lidarConfig &new_config, uint32_t level)
    {
        config_ = new_config;
    ...
    bool Vanjee718MiniLidarProtocol::protocl(unsigned char *data, const int len)
    {
        if(data[13] != 0x11)
        {
            return false;
